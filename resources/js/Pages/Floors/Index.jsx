...
const Index = ({ columns }) => {
    ...
    const destroy = id => {
        onConfirm(t('confirm_delete_floor'))
            .then(() => {
        ...
        ...tableColumns,
        {
            ...
            Cell: ({ row }) => (
                <div className="flex gap-x-2">
                    {hasPermission('floors edit') && (
                        <RoundedButtonLink
                            href={route('floors.edit', row.original.floor_id)}
                            icon={<FaPencil />}
                            popoverText={t('Edit')}
                            outline
                        />
                    )}
                    {hasPermission('floors delete') && (
                        <RoundedButton
                            icon={<FaTrash />}
                            popoverText={t('delete')}
                            buttonType="danger"
                            onClick={() => destroy(row.original.floor_id)}
                            outline
                        />
                    )}
                </div>
        ...
        <>
            <AuthenticatedLayout>
                ...
                <div className="rounded-lg bg-white">
                    <DataTable
                        permissionModule="floors"
                        columns={tableColumns}
                        duplicateUrl={route('floors.create')}
                        pdfUrl={route('floors.pdf')}
                        exportUrl={route('floors.export')}
                        url={route('floors.datatable')}
                        deleteUrl={route('floors.bulkDelete')}
                        editUrl={route('floors.bulkEdit')}
                        primaryKey={'floor_id'}
